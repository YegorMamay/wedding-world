...
((w, d, $, ajax) => {
    ...
    $(() => {
        ...
        buyOneClick('.one-click-ru, .one-click', '[data-field-id="field11"]', 'h1');
        // On Copy
        $d.on('copy', addLink);
    ...
    const reviews = (container) => {
        ...
        if (element.children().length > 1 && typeof $.fn.slick === 'function') {
            element.slick({
                ...
                autoplaySpeed: 3000,
                arrows: true,
                prevArrow: '<button type="button" class="slick-prev"><svg class="slider-icon"><use xlink:href="#arrow_left"></use></svg></button>',
                nextArrow: '<button type="button" class="slick-next"><svg class="slider-icon"><use xlink:href="#arrow_right"></use></svg></button>',
                dots: false,
    ...
    let addLightBoxHandlerForImage = function addLightBoxHandlerForImage(sliderContainer) {
